    ...
    while (cmd) {
      switch (cmd.type) {
        ...
        case BlockTypes.JOIN_CHARTS: {
          ...
          let right_pattern = ''
          const patternBlock = cmd.getInputTargetBlock('PATTERN')
          if (patternBlock && (patternBlock.type === BlockTypes.PATTERN_ROW || patternBlock.type === BlockTypes.ADD_ROW)) {
            right_pattern = String(patternBlock.getFieldValue('PATTERN') ?? '').trim()
          }
          if (!right_chart_name) {
            errors.push('Join chart requires a chart name.')
          } else {
            commands.push({ op: 'join', left_chart_name: name, right_chart_name, right_pattern } as const)
          }
          break
        }
        default:
